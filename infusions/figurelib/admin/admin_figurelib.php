...
if (!empty($result)) {
	...
	if (isset($_POST['save_figure'])) {
        ...
        if (isset($_POST['del_image'])) {
            if (!empty($data['figure_images_image']) && file_exists(IMAGES_FIGURES.$data['figure_images_image'])) {
                unlink(IMAGES_FIGURES.$data['figure_images_image']);
            }
            if (!empty($data['figure_images_thumb']) && file_exists(THUMBS_FIGURES.$data['figure_images_thumb'])) {
                unlink(THUMBS_FIGURES.$data['figure_images_thumb']);
            }
            
            $data['figure_images_image'] = "";
            $data['figure_images_thumb'] = "";
...
// File Field "Images" /////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	*/
        $resultimage = dbquery("SELECT * FROM ".DB_FIGURE_IMAGES." WHERE figure_images_image_id='".(isset($_POST['figure_id']) ? $_POST['figure_id'] : $_GET['figure_id'])."'");
        if (dbrows($resultimage)) {
            $data2 = dbarray($resultimage);
        } else {
            redirect(FUSION_SELF.$aidlink);
        }
		
		if ($data2['figure_images_image'] != "" && $data2['figure_images_thumb'] != "") {
        $image_thumb = get_figure_image_path($data2['figure_images_image'], $data2['figure_images_thumb']);
        if (!$image_thumb) {
            $image_thumb = IMAGES."imagenotfound70.jpg";	
        }
        echo "<div class='row'>\n";
        echo "<div class='col-xs-12 col-sm-6'>\n";
		echo "<label><img class='img-responsive img-thumbnail' src='".$image_thumb."' alt='".$locale['figure_588']."' /><br />\n";
        echo "<input type='checkbox' name='del_image' value='y' /> ".$locale['delete']."</label>\n";
        echo "</div>\n";
        echo "<div class='col-xs-12 col-sm-6'>\n";
        
        echo "</div>\n</div>\n";
        echo "<input type='hidden' name='figure_images_image' value='".$data2['figure_images_image']."' />\n";
        echo "<input type='hidden' name='figure_images_thumb' value='".$data2['figure_images_thumb']."' />\n";
    } else {
        ...
        echo "<div class='small m-b-10'>".sprintf($locale['figure_035'],
                                                  parsebytesize($asettings['figure_photo_max_b']))."</div>\n";
